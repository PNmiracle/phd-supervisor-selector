...
国内（中国大陆）学校必须填入**具体院系/专业的博士招生信息页面**，禁止只填学校研究生院首页或学校研招网首页。

#### 有效链接标准（按优先级）

1. `XX学院 博士研究生招生简章`（当年或最近一年发布的）
2. `XX学院 博士招生专业目录`
3. `XX专业 博士研究生培养方案`
4. 院系官网「研究生教育」或「博士招生」栏目页

#### 无效链接（禁止填入）

- 学校研究生院首页（如 `https://grad.xxx.edu.cn`）
- 学校研招网首页（如 `https://yz.xxx.edu.cn`）
- 缺少具体院系/专业信息的通用招生页面
- 通过猜测 URL 构造的链接（如在导师主页域名后拼接 `/phd` 或 `/admission`）

#### 强制搜索流程（不可跳过）

「博士申请信息」字段的填写必须按以下步骤完成，缺一步视为未完成：

1. **搜索**：百度搜索 `XX大学 XX学院 博士招生简章 2026` 或 `XX大学 XX学院 博士研究生招生目录`
2. **验证**：从搜索结果中打开至少一个候选页面，通过 WebFetch 确认页面内容包含具体的招生信息（招生人数、报考条件、考试科目、申请材料清单等）
3. **层级确认**：检查页面 URL 路径，确认不是学校级别的通用入口。院系级别 URL 通常包含学院标识（如 `sociology`、`cs`、`psychology`），而不是 `grad`/`yjsy`/`yz` 等研究生院层级
4. **输出证据**：在对话中明确输出：「已确认：XX大学XX学院博士招生简章（YYYY年），URL=xxx」后再填入表格

#### 禁止行为（视为偷懒）

- ❌ 所有国内学校填写同一个研究生院链接
- ❌ 用导师主页域名推测博士申请页面 URL
- ❌ 用百度搜索结果摘要确认，不打开页面 WebFetch 验证
- ❌ 不同院系共用同一个学校级招生链接
