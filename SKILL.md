...
**默认输出列**：`导师`, `Location`, `学校名字`, `QS排名`, `美国USNEWS排名`, `Department`, `导师主页`, `博士申请信息`, `其他导师信息`, `备注`, `待确认导师`
...
# 待确认导师为 `新加待check` 的行（这些是待人工审核的行，自查时可修改，其他时候只读）
pending_check = [r for r in all_records if r["fields"].get("待确认导师") == "新加待check"]

# 可自由编辑的行：选导意向为空 且 待确认导师不为 `新加待check`
editable = [r for r in all_records
            if not r["fields"].get("选导意向（点击选择）")
            and r["fields"].get("待确认导师") != "新加待check"]

# 自查模式下（用户说"检查新加导师"时）：
# 可编辑 pending_check 中的记录，但仍不可动 locked 中的记录
```

**总结：**
| 场景 | locked（选导意向非空） | pending_check（待确认导师=新加待check） | editable（其余） |
|------|:---:|:---:|:---:|
| 新增导师 | 🚫 不操作 | 🚫 不操作 | ✅ 可操作 |
| 自查修正 | 🚫 不操作 | ✅ 可修改 | 🚫 不操作 |
| 普通编辑 | 🚫 不操作 | 🚫 不操作 | ✅ 可操作 |
...
| 第二段 | 研究方向 | 从导师主页提炼核心方向；可摘录主页原文作为出处（如 `"research focuses on consumer judgment and decision-making"`） |
| 第三段 | 匹配度标识 | 三选一，见下表 |
...
| 标识语 | 触发条件 |
|--------|----------|
| `建议多看看呢～` | 方向多重重叠 + 顶刊发表 + 正在招博士/有成熟指导记录 |
| `比较匹配～` | 研究方向与学生1-2个目标方向直接相关，有合理发表记录 |
| `可以备选一下～` | 方向有一定交叉但非核心对应，或资历/职称偏早期 |

**禁止事项**：
- 禁止在备注里使用任何 emoji 符号（🔥👍👀⭐✅❌ 等）
- 禁止使用主观评价用语如 `高度匹配`、`很匹配`、`完美匹配`、`强匹配`、`弱相关`
- 禁止省略匹配度标识（每条备注结尾必须带一个）
- 描述事实，不做评判
...
正确示例：
- `教授；决策神经科学、风险与社会决策、无创脑刺激；这个系老师主要做临床和辅导实践方向，只有这一位和消费者决策搭边；比较匹配～`
- `副教授；消费者判断与决策；这个系以社会认知和神经犯罪学为主，就这一位和消费者行为能搭上；比较匹配～`
...
> ⚠️ **这是学生自己操作的一列！AI 绝对不能读写！**

`选导意向（点击选择）` 列是学生在 Vika 中手动选择的（`优先套磁` / `第二批套磁` / `完全不考虑`），AI **绝对禁止**：

- ❌ **禁止写入** — 新增/修改记录时**永远不要带** `选导意向（点击选择）` 字段
- ❌ **禁止清空** — 不允许把已有值设为空
- ❌ **禁止修改**该行的任何字段（选导意向非空 = 整行锁定）
- ❌ **禁止删除**该行
- ✅ **仅可读取**，用于过滤

这些是学生已经审阅并反馈过的记录，任何修改都会造成数据损失。**违反此规则等同于破坏学生数据。**

**操作前必须：**
1. 先 GET 所有记录
2. 过滤掉 `选导意向（点击选择）` 非空的行
3. 仅对 `选导意向` 为空的行执行增删改操作
4. 写入记录时**字段列表中绝不包含** `选导意向（点击选择）`

### 🏷️ 待确认导师质量门控机制

Vika 表中存在 `待确认导师` 列（文本/单选类型）。这是一个 **AI 写入 → AI 自查 → 人工审核 → 学生可见** 的四段式质量门控。

#### 写入规则（AI 添加新导师时强制执行）

**每次 AI 新增导师记录时，必须同步将 `待确认导师` 设为 `新加待check`。** 这标记该记录为"AI 新增、待人工复核"状态。

```python
new_records = [
    {"fields": {
        "导师": "张三",
        "Department": "心理学院(XX大学)",
        "备注": "教授；决策研究；比较匹配～",
        "待确认导师": "新加待check"  # ← 所有新增记录必须带这个
    }},
]
```

- 仅新增记录时设为 `新加待check`
- 更新已有记录（补充备注、修正信息等）时**不修改** `待确认导师` 的值
- `待确认导师` 为 `新加待check` 的记录，对学生端（学生视图）**不可见**，只有维护人视角可见

#### 检查规则（AI 自查，用户说"检查新加导师"时触发）

当用户说"检查新加的"、"check 一下"、"自查新加导师"等时，AI 对 `待确认导师` 为 `新加待check` 的所有记录执行以下检查：

```
[ ] 1. 导师主页 URL 是否可访问且确认为个人页面（非系列表页）
[ ] 2. 导师姓名、职称、院系是否与主页一致
[ ] 3. 研究方向与学生方向的匹配度是否合理（不过度夸大也不过度保守）
[ ] 4. 备注格式是否符合三段式规范（职称；研究方向；匹配度标识）
[ ] 5. 备注中是否有禁止内容（✅ emoji、主观评价、方向偏向、报名信息等）
[ ] 6. 博士申请信息 & 其他导师信息 URL 是否有效、域名是否与学校一致
[ ] 7. 是否有遗漏的风险标注（需确认带博、教学岗风险、更新偏旧等）
[ ] 8. 排除 Emeritus/退休/约70岁以上/Visiting 等不应加入的导师
[ ] 9. 该导师是否已存在于选导意向非空的记录中（去重）
```

检查完成后输出结果清单：
- ✅ **通过** X 条 — 无需修改
- ⚠️ **修正** Y 条 — 列出具体问题和修正内容
- ❌ **建议删除** Z 条 — 列出原因

**检查过程中只修改 `待确认导师` 为 `新加待check` 的记录**，其他记录（`待确认导师` 为空或选导意向非空）不动。

#### 放行规则（人工操作，AI 提醒）

AI 自查完成后提醒用户：

> 自查完毕。请人工抽检后，手动将确认通过的记录的 `待确认导师` 清空。清空后该记录才会对学生可见。

- AI **不自动清空** `待确认导师` — 必须人工确认后手动操作
- 人工清空后，该记录进入正常流程（受选导意向保护规则约束）
- 如果检查发现问题并修正了记录，`待确认导师` 保持 `新加待check`，等待人工最终确认

#### 与其他保护规则的关系

| 规则 | 保护范围 | 触发条件 |
|------|---------|---------|
| 选导意向保护 | 禁止修改/删除 | `选导意向（点击选择）` 非空 |
| 待确认导师门控 | 对学生不可见 | `待确认导师` = `新加待check` |
| 表格隔离 | 禁止操作其他表 | 始终 |

两条保护规则独立运作：一条记录可以同时满足两者（如学生选导后又新增了待check记录），此时以最严格规则为准（禁止修改）。
...
### 🚫 严禁触碰选导意向列（最高优先级）
`选导意向（点击选择）` 是学生手动操作的列，AI **绝对不能写入、清空、修改、删除**。新增记录时字段列表中绝不包含此列。详见「操作权限 → 选导意向保护」。
...
### 新增记录必须标记待确认导师
每次新增导师记录时，`待确认导师` 字段必须设为 `新加待check`。详见「操作权限 → 待确认导师质量门控机制」。
...
[ ] 8. 待确认导师: 新增记录必须设为 `新加待check`
[ ] 9. ⚠️ 绝不写入选导意向列: 字段列表中不包含 `选导意向（点击选择）`
```
